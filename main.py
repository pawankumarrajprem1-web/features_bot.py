"""
====================================================================
🤖 STANDALONE ADVANCED FEATURE DASHBOARD BOT (Single File)
--------------------------------------------------------------------
Author: Amar (Aryan)
Description: Complete single-file standalone bot for testing the 
             12-Feature Management System, Sub-Menus, Dynamic 
             Negative Marking, and Interactive Dashboard using pyTelegramBotAPI.
====================================================================
"""
...
TELEGRAM_BOT_TOKEN = os.getenv("BOT_TOKEN", "Y")
...
logger = logging.getLogger("standalone_feature_bot")
...
# यूजर सेटिंग्स का डेटाबेस (मेमोरी स्टोरेज)
...
def build_negative_marking_submenu(user_id: int) -> InlineKeyboardMarkup:
    """नेगेटिव मार्किंग के लिए एडवांस्ड सब-मेनू।"""
    features = get_user_features(user_id)
...
def get_final_quiz_execution_config(user_id: int) -> dict:
    ...
    return {
        ...
        "quiz_mode_type": features["quiz_mode_type"],
        "status": "LOCKED_AND_READY"
    }


# ====================================================================
# 4. BOT INITIALIZATION & HANDLERS REGISTRATION
# ====================================================================
def main():
    ...
    @bot.message_handler(commands=['start', 'feature', 'features', 'setting', 'settings'])
    def feature_command_handler(message):
        ...
        try:
            ...
            logger.error(f"Error opening feature dashboard: {e}")
    ...
    def handle_custom_user_input(message):
        ...
        try:
            ...
            bot.reply_to(message, "⚠️ अमान्य मान (Invalid value)! कृपया केवल संख्या (जैसे 0.15 या 0.40) टाइप करें:")
    ...
    def feature_callback_router(call):
        ...
        try:
            if data == 'feat_toggle_paid_quiz':
                ...
                features["quiz_mode_type"] = not features["quiz_mode_type"]
            ...
            if data == 'sub_neg_menu':
                ...
                bot.edit_message_text(
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    text="⚙️ *STANDALONE FEATURE DASHBOARD* ⚙️",
                    parse_mode="Markdown",
                    reply_markup=build_main_dashboard_keyboard(user_id)
                )
                ...
                bot.edit_message_text(
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    text="❌ *Feature Dashboard Closed.*\nदुबारा खोलने के लिए `/feature` टाइप करें।",
                    parse_mode="Markdown"
                )
                return
                ...
                success_text = (
                    ...
                    "_यह स्टैंडअलोन टेस्ट बोट पूरी तरह से काम कर रहा है!_ ✨"
                )
                bot.edit_message_text(
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                ...
                bot.answer_callback_query(call.id, "🚀 Settings locked!")

        except Exception as e:
            logger.error(f"Error in feature callback router: {e}")
            bot.answer_callback_query(call.id, "⚠️ An error occurred, please try again.")

    logger.info("Standalone Feature Bot is up and running via polling...")
